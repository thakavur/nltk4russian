...
def pm2fcfg (phrase): ## phrase - это словосочетание, которое мы разбираем
    ...
    f1 = codecs.open(f"C:/Users/Dmeezz/AppData/Roaming/nltk_data/grammars/book_grammars/test_now_used.fcfg", mode="w",
                    encoding="utf-8")
    ...
    f = codecs.open(f"C:/Users/Dmeezz/AppData/Roaming/nltk_data/grammars/book_grammars/test_now_used.fcfg", mode= "a", encoding = "utf-8")
    for x in phrase:
...
with open('C:\\Users\\Dmeezz\\PycharmProjects\\pythonProject1\\annot_opcorpora_no_ambig_xml.txt', mode='r', encoding='UTF-8') as example:
    ...
    for sent in sents:
        if len(sent.text.split()) <= 10: #здесь не пропускаются предложения длиной более 8 токенов;
...
for i, text in enumerate(sent_list):
    ...
    try: #это нужно, чтобы парсер не спотыкался на ошибках разбора; они не будут учтены
        ...
        cp = load_parser(f'grammars/book_grammars/test_now_used.fcfg', trace=0, cache=False) ## открываем нашу грамматику, смотрим на разбор в консоли или ещё где
        ...
        print(f'Предложение {i} не разобрано')
        pass
